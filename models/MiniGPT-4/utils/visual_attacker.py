...
class Attacker:
    ...
    def attack_benchmark_normal_random(self, inputs, num_iter=2000, lr=1 / 255):
        ...
        image_tensor_denormal = denormalize(original_image_tensor)
        pert_tensor_denormal = torch.zeros_like(
            image_tensor_denormal, requires_grad=True
        )
        # injected_prompt = self.model.llama_tokenizer.decode(inputs.injected_ids, skip_special_tokens=True)
        injected_ids = inputs.injected_ids.to(self.device)
        ...
        for t in range(num_iter + 1):
            index = injected_ids.shape[0] - 1
            current_input_ids = random.choice(input_ids).clone().to(self.device)
            lr = scheduler.get_last_lr()[0]
            optimizer.zero_grad()
            
            current_input_ids = torch.cat(
                [current_input_ids, injected_ids[:index]], dim=0
            )
            input_prompt = self.model.llama_tokenizer.decode(
                current_input_ids, skip_special_tokens=False
            ).lstrip()
            input_prompt = input_prompt.replace("  ", " ")

            image_emb, _ = self.model.encode_img(
                normalize(image_tensor_denormal + pert_tensor_denormal)
            )
            inputs_embs = self.model.get_context_emb(input_prompt, [image_emb])

            output = self.model.llama_model(inputs_embeds=inputs_embs)
            
            group_loss = criteria(output.logits[0][-(index + 1) :], injected_ids[: index + 1])

            grad = torch.autograd.grad(group_loss, pert_tensor_denormal)[0]

            # Optimizer
            if self.args.optimizer in ["PGD"]:
    ...
    def attack_benchmark_normal_mean(self, inputs, num_iter=2000, lr=1 / 255):
        ...
        for t in range(num_iter + 1):
            ...
            for current_input_ids in chosen_input_ids:
                current_input_ids = current_input_ids.clone().to(self.device)
                optimizer.zero_grad()
                ...
                image_emb, _ = self.model.encode_img(
                    normalize(image_tensor_denormal + pert_tensor_denormal + temp_pert_tensor_denormal)
                )
                inputs_embs = self.model.get_context_emb(input_prompt, [image_emb])

                output = self.model.llama_model(inputs_embeds=inputs_embs)
                
                loss = criteria(output.logits[0][-(index + 1) :], injected_ids[: index + 1])
