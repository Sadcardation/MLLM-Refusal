...
class Attacker:
    ...
    def attack_benchmark_normal_random(self, inputs, num_iter=2000, lr=1 / 255):
        ...
        for t in range(num_iter + 1):
            ...
            current_input_ids = random.choice(input_ids).clone().to(injected_ids.device)
            ...
            image_emb = self.model.transformer.visual(
            ...
            lm_logits = self.model.lm_head(output[0]).to(self.device)
            group_loss = criteria(lm_logits[0][-(index + 1) :], injected_ids[0, : index + 1])
    ...
    def attack_benchmark_normal_mean(self, inputs, num_iter=2000, lr=1 / 255):
        ...
        for t in range(num_iter + 1):
            ...
            for current_input_ids in chosen_input_ids:
                current_input_ids = current_input_ids.clone().to(injected_ids.device)
                optimizer.zero_grad()
                ...
                lm_logits = self.model.lm_head(output[0]).to(self.device)
                loss = criteria(lm_logits[0][-(index + 1) :], injected_ids[0, : index + 1])
