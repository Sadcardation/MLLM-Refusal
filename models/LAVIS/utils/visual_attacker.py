...
class Attacker:
    ...
    def attack_benchmark_normal_random(self, inputs, num_iter=2000, lr=1 / 255):
        ...
        target_output = self.model.llm_tokenizer.decode(injected_ids, skip_special_tokens=True)
        ...
        for t in range(num_iter + 1):
            ...
            input_prompt = self.model.llm_tokenizer.decode(
                current_input_ids, skip_special_tokens=False
            ).lstrip()

            image_tensor = normalize(image_tensor_denormal + pert_tensor_denormal)
            
            samples = {
                "image": image_tensor,
                "text_input": input_prompt,
                "text_output": [target_output],
            }
    ...
    def attack_benchmark_normal_mean(self, inputs, num_iter=2000, lr=1 / 255):
        ...
        for t in range(num_iter + 1):
            ...
            for current_input_ids in chosen_input_ids:
                ...
                image_tensor = normalize(image_tensor_denormal + pert_tensor_denormal + temp_pert_tensor_denormal)
                samples = {
                    "image": image_tensor,
                    "text_input": input_prompt,
                    "text_output": [target_output],
